...
# Code was based on paddlepaddle/vision_transformer.py
...
class Attention(nn.Layer):
    ...
    def forward(self, x):
        ...
        q = self.query(x).reshape([0,0,self.num_heads,self.head_dim]).transpose((0, 2, 1, 3))
        k = self.key(x).reshape([0,0,self.num_heads,self.head_dim]).transpose((0, 2, 1, 3))
        v = self.value(x).reshape([0,0,self.num_heads,self.head_dim]).transpose((0, 2, 1, 3))
...
class PatchEmbed(nn.Layer):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768,stride_size=None):
        super().__init__()
        ...
        self.proj = nn.Conv2D(
            in_chans, embed_dim, kernel_size=patch_size, stride=patch_size if stride_size is None else to_2tuple(stride_size))
